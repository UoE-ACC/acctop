...
def display_network_usage():
    ...
    sent_width = 20
    max_bytes_sent = max(stats.bytes_sent for stats in net_io.values())
    max_bytes_recv = max(stats.bytes_recv for stats in net_io.values())
    max_packets_sent = max(stats.packets_sent for stats in net_io.values())
    max_packets_recv = max(stats.packets_recv for stats in net_io.values())
    print(max_bytes_sent, max_bytes_recv, max_packets_sent, max_packets_recv)
    input()
    recv_width = 20
